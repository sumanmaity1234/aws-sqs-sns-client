...
exports.createServer = (aws, app, ui) => {
  // const app = express()
  ...
  app.get('/dynamodb', asyncMiddleware((req, res) => {
  ...
  app.get('/dynamodb/api/tables', (req, res) => {
    return listAllTables(null, [])
      .then(data => {
        res.send(data)
      })
  ...
  app.get('/dynamodb/create-table', (req, res) => {
  ...
  app.delete('/dynamodb/tables/:TableName', asyncMiddleware((req, res) => {
  ...
  app.delete('/dynamodb/tables/:TableName/all', asyncMiddleware((req, res) => {
  ...
  app.get('/dynamodb/tables/:TableName/get', asyncMiddleware((req, res) => {
    const TableName = req.params.TableName
    if (req.query.hash) {
      if (req.query.range) {
        return res.redirect(
          `/dynamodb/tables/${encodeURIComponent(TableName)}/items/${
            encodeURIComponent(req.query.hash)},${encodeURIComponent(req.query.range)}`
        )
      } else {
        return res.redirect(`/dynamodb/tables/${
  ...
  app.get('/dynamodb/tables/:TableName', asyncMiddleware((req, res) => {
    const TableName = req.params.TableName
  ...
  app.get('/dynamodb/tables/:TableName/items', asyncMiddleware((req, res) => {
    const TableName = req.params.TableName
    req.query = pickBy(req.query)
  ...
  app.get('/dynamodb/tables/:TableName/meta', asyncMiddleware((req, res) => {
    const TableName = req.params.TableName
    return Promise.all([
  ...
  app.delete('/dynamodb/tables/:TableName/items/:key', asyncMiddleware((req, res) => {
    const TableName = req.params.TableName
    return describeTable({ TableName })
      .then(result => {
        const params = {
  ...
  app.get('/dynamodb/tables/:TableName/add-item', asyncMiddleware((req, res) => {
    const TableName = req.params.TableName
    return describeTable({ TableName })
      .then(result => {
  ...
  app.get('/dynamodb/tables/:TableName/items/:key', asyncMiddleware((req, res) => {
    const TableName = req.params.TableName
    return describeTable({ TableName })
      .then(result => {
        const params = {
  ...
  app.put(
    '/dynamodb/tables/:TableName/add-item',
    bodyParser.json({ limit: '500kb' }),
    asyncMiddleware((req, res) => {
      const TableName = req.params.TableName
      return describeTable({ TableName })
  ...
  app.put(
    '/dynamodb/tables/:TableName/items/:key',
    bodyParser.json({ limit: '500kb' }),
    asyncMiddleware((req, res) => {
      const TableName = req.params.TableName
      return describeTable({ TableName })
